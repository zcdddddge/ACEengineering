...
void PY_Motor(G_t *G )
{
    ...
    {
        G->PitchMotor.ExpRadian -= G->Rc->RC_ctrl->KV.y * 0.01f * 0.6f;
        G->PitchMotor.ExpRadian = limit(G->PitchMotor.ExpRadian, 30, -30);
    }
    ...
    G->Can_Send_Gimbal(0, 0, G->YawMotor.SPID.Out, G->PitchMotor.SPID.Out);
