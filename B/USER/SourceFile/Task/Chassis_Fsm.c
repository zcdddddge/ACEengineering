...
{
    /***************************************底盘选择**********************************/
    if (Chassis.RC->state.Independent == 0 || Chassis.RC->state.Auto_Run == 0 || Chassis.RC->state.Rotation == 0 || Chassis.RC->state.Wiggle == 0)
    {
        Chassis.Poweroff(&Chassis.C);
    }
    ...
    if (Chassis.RC->state.Auto_Run == 1)
    {
        Chassis.Auto_run(&Chassis.C ,Chassis.RC->KM_X.Output, Chassis.RC->KM_Y.Output, Chassis.RC->KM_Z.Output);
    }
...
{
    Send_Ctrl_To_Board(Chassis.RC->state.Clap_Init, Chassis.RC->state.Auto_One_Box, Chassis.RC->state.Auto_Two_Box, Chassis.RC->state.Auto_Three_Box, Chassis.RC->state.Flip_Reset, Chassis.RC->state.All_Reset);
}
