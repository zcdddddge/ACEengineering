...
void Send_Crtl_To_Board(int16_t ch0, u8 Smooth_L, u8 Smooth_R)
{
    ...
    temp[1] = ch0;
    temp[2] = ch0 >> 8;
    temp[3] = Sensor.Smooth_L;
    temp[4] = Sensor.Smooth_R;
    temp[5] = 0;
    temp[6] = 0;
    temp[7] = 0xCE;
...
void Board_Communi_Updata(void)
{
    ...
    switch (Board_Communi.Rc[17])
    {
        ...
        case 2:
        {
            ...
            Board_Communi.state.Clap_Init = Board_Communi.Rc[0];      //夹取抬升初始化
            Board_Communi.state.Auto_One_Box = Board_Communi.Rc[1];   //夹取一箱
            Board_Communi.state.Auto_Two_Box = Board_Communi.Rc[2];   //夹取两箱
            Board_Communi.state.Auto_Three_Box = Board_Communi.Rc[3]; //夹取三箱
            Board_Communi.state.Flip_Reset = Board_Communi.Rc[4];     //翻转复位
            Board_Communi.state.All_Reset = Board_Communi.Rc[5];      //上装复位
        }
    }
}
